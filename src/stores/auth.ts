...
export const useAuthStore = defineStore('auth', () => {
  ...
  const login = async (credentials: LoginRequest): Promise<boolean> => {
    ...
    const userResponse = response.data?.token 

    if (!userResponse) {
      error.value = 'Error with token generation'
      loading.value = false
      return false
    }
    
    localStorage.setItem('authToken', userResponse)
    const idResponse = await authApi.verifyToken({token: userResponse});
    
    if (idResponse.error) {
      error.value = idResponse.error
      loading.value = false
      return false
    }
    
    const id = idResponse.data?.userId;

    const userData: User = id ? 
    { username: credentials.username, _id: id , password: credentials.password } : 
    {username: '', _id: '', password: ''}
    user.value = userData
    saveToStorage(userData)
